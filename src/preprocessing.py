...
def preprocess_data(df):
    ...
    # Standardize missing values. Covers common representations of missing data
    ...
    # Columns that must exist and be numeric for analytics
    numeric_cols = [
    ...
    # Employment rates between 0 and 100
    emp_cols = ["employment_rate_overall", "employment_rate_ft_perm"]
    for col in emp_cols:
        df.loc[(df[col] < 0) | (df[col] > 100), col] = pd.NA


    # Define critical columns for analytics
    critical_cols = [
        "year",
        "university",
        "school",
    ...
    invalid_df = df[~df.index.isin(clean_df.index)]
...
if __name__ == "__main__":
    ...
    # Optional: save invalid rows for inspection
